...
public class InsertDocument extends InsertWhenDocument {
  ...
  Mono<Void> verifyBalanceSuficien(WalletEntity wallet, MovementEntity movementWallet) {
    return Mono.just(wallet)
        .flatMap(walletFlujo -> {
          if (walletFlujo.getTotalBalance().doubleValue() < movementWallet.getAmount().doubleValue()) {
            return Mono.error(new ErrorResponseException(EX_ERROR_BALANCE_INSUFICIENT, HttpStatus.CONFLICT.value(),
                HttpStatus.CONFLICT));
          } else {
            return Mono.empty();
          }
  ...
  @Override
  Mono<Void> updateWalletOrigin(WalletEntity wallet, MovementEntity movementWallet, WalletRepository walletRepository) {
    return Mono.just(wallet)
        .flatMap(walletFlujo -> {
          BigDecimal newValue = walletFlujo.getTotalBalance().subtract(movementWallet.getAmount());
          walletFlujo.setTotalBalance(newValue);
          return walletRepository.save(walletFlujo);
        }).then();
  }

  @Override
  Mono<Void> updateWalletDestin(WalletEntity wallet, MovementEntity movementWallet,
